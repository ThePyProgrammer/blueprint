...
- `config/evidence.toml` — existing evidence classifications
...
2. Spawn a `blueprint:adr-evidence-auditor` agent with:
   ...
   - Current `config/evidence.toml`
   - Current dependency manifests (package.json, requirements.txt, etc.)
   - Project root path
3. Present the evidence audit report
4. Offer to update `config/evidence.toml` with new classifications
5. Update `config/state.toml` — set `last_evidence_audit` to today
...
Updates `config/evidence.toml` and the ADR metadata.

## Config File: `config/evidence.toml`
