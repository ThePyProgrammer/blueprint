...
- `state.toml` — ADR directory location (if previously detected)
- `relationships.toml` — existing ADR relationships (for impact awareness)
...
7. **Update `relationships.toml`** — add the new ADR as a node
8. **Update `state.toml`** — set `last_adr_created` to today
