...
- `config/contexts.toml` — bounded contexts with owners
- `config/relationships.toml` — existing ADR graph
...
2. **Determine affected parties:**
   - Read `config/contexts.toml` — which contexts are affected? Who owns them?
...
7. **Update `config/state.toml`** — set `last_advice_process` to today
